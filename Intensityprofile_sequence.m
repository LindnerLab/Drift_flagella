%% QUANTIFICATION OF THE MIGRATION OF FLUORESCENT FLAGELLA AS A FUNCTION OF Z - IMAGE SEQUENCE
%
%~~~~~  GENERAL DESCRIPTION   : 
%
...
%    ** RAW VALUES AND FIT VALUES ARE SAVED (BOTH MEANS AND ABSCISSA RANGES)
...
for k = 1 : numFiles
    ...
    [x,y]=size(Im); % Grid of the image (for example 2048 x 2048)
    X = 1:x; % Vector containing each pixel along any line on the image (NB: that only works for SQUARE IMAGES)
    i= im2double(Im); % intensity value of each pixel
    Vectors = mat2tiles(i,[1,y]); % Pixel intensity values for each vertical line in the image (1 column of this matrix = 1 vertical line)
    for p = 1:x
        Means(p) = mean(Vectors{p,1}); % Averaging intensity values over X for a given position in Y (assuming drift occurs in Y)
...
% ** TESTED FITS AND RANGES ---------------------------------------------
    for m = 1:4
        for n = 1:4
            ...
            % Gof = "Goodness of fit" (records the fit rsquare for example). 
...
TxtLF = sprintf('%.1f',LowestFrame);
TxtMF = sprintf('%.1f',MidFrame);
TxtHF = sprintf('%.1f',HighestFrame);
% * PLOTTING
figure()
hold on
